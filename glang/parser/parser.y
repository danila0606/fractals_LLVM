...
%token WHILE             "while"   
       ...
       DIV               "/"
       PRINT             "print"
    //    GET_TIME          "get_time_milliseconds"   
    //    FLUSH             "flush"                   
    //    PUT_PIXEL         "put_pixel"            
    //    INIT              "init"                    
    //    IS_WINDOW_OPEN    "is_window_opened" 
...
/* graphic :  IS_WINDOW_OPEN LRB ARGS RRB SCOLON   {   auto&& cur_func_args = driver->cur_args;
                                                    auto&& scope = driver->curscope;
                                                    $$ = std::make_shared<glang::GraphicNode>($3, glang::graphic_type::is_window_open);
                                                    cur_func_args.clear();
                                                };
        | INIT LRB ARGS RRB SCOLON              {   auto&& cur_func_args = driver->cur_args;
                                                    auto&& scope = driver->curscope;
                                                    $$ = std::make_shared<glang::GraphicNode>($3, glang::graphic_type::init);
                                                    cur_func_args.clear();
                                                };
        | PUT_PIXEL LRB ARGS RRB SCOLON         {   auto&& cur_func_args = driver->cur_args;
                                                    auto&& scope = driver->curscope;
                                                    $$ = std::make_shared<glang::GraphicNode>($3, glang::graphic_type::put_pixel);
                                                    cur_func_args.clear();
                                                };
        | FLUSH LRB ARGS RRB SCOLON             {   auto&& cur_func_args = driver->cur_args;
                                                    auto&& scope = driver->curscope;
                                                    $$ = std::make_shared<glang::GraphicNode>($3, glang::graphic_type::flush);
                                                    cur_func_args.clear();
                                                };
        | GET_TIME LRB ARGS RRB SCOLON          {   auto&& cur_func_args = driver->cur_args;
                                                    auto&& scope = driver->curscope;
                                                    $$ = std::make_shared<glang::GraphicNode>($3, glang::graphic_type::get_time);
                                                    cur_func_args.clear();
                                                };
                                            */
...
FUNCCALL:       VARIABLE LRB ARGS RRB          { 
                                                        ...
                                                        auto&& node = scope->get_declaration($1);
                                                        if (!node) {
                                                            node = std::make_shared<glang::DeclarationFuncNode>($1, cur_func_args);
                                                        }
